...
<body>
  ...
  <footer class="site-footer" id="contact"><div class="wrap footer-grid"><div><strong>Финские Новости</strong><p>Русскоязычные пересказы новостей Финляндии со ссылками на первоисточники.</p><a href="/about">О проекте и конфиденциальность</a></div><form class="contact-form" action="/contact" method="post"><h2>Связаться с редакцией</h2><label>Имя<input name="name" maxlength="80" required></label><label>E-mail<input name="email" type="email" maxlength="254" required></label><label>Сообщение<textarea name="body" maxlength="3000" required></textarea></label><button type="submit">Отправить сообщение</button></form></div></footer>
  ${interestModal}
