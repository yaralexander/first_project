...
function renderAdminPage({
  ...
  const activeTab = new Set(['stats', 'articles', 'comments', 'duplicates', 'audit']).has(tab) ? tab : 'stats';
  ...
  const tabNav = `<nav class="admin-tabs" aria-label="Разделы админ-панели">${[['stats', '📊 Статистика'], ['articles', '📰 Статьи'], ['comments', '💬 Комментарии'], ['duplicates', '🔎 Повторы'], ['audit', '🛡 Журнал']].map(([name, label]) => `<a class="${activeTab === name ? 'active' : ''}" href="/admin?tab=${name}">${label}</a>`).join('')}</nav>`;
