...
<div class="util-bar"><div class="wrap"><div class="util-left translation-status" data-translation-status data-mode="loading"><span class="translation-status-dot" aria-hidden="true"></span><span data-translation-label>Проверяем режим перевода…</span></div><div class="util-right"><button class="utility-button" type="button" data-font-step="-0.10" aria-label="Уменьшить текст">A−</button><button class="utility-button utility-button--scale" type="button" data-font-reset aria-label="Обычный размер текста">100%</button><button class="utility-button" type="button" data-font-step="0.10" aria-label="Увеличить текст">A+</button><button class="utility-button utility-theme-status" type="button" data-theme-toggle aria-label="Переключить цветовую тему">🌙 Тёмная</button></div></div></div>
  ...
  ${interestModal}
<nav class="mobile-bottom-nav" aria-label="Мобильная навигация"><a href="/"><i>⌂</i><span>Главная</span></a><a href="/search"><i>⌕</i><span>Поиск</span></a><a href="/#feed-heading"><i>♧</i><span>Лента</span></a><a href="#category-nav"><i>⊞</i><span>Разделы</span></a><button class="mobile-theme-status" type="button" data-theme-toggle aria-label="Переключить цветовую тему"><i>🌙</i><span>Тёмная</span></button></nav>
  ${themeScript}
  <script>(function(){var box=document.querySelector('[data-translation-status]'),label=document.querySelector('[data-translation-label]');if(!box||!label)return;function show(mode){var ai=mode==='ai';box.dataset.mode=ai?'ai':'google';label.textContent=ai?'🇫🇮 → 🇷🇺 AI-пересказ работает':'🇫🇮 → 🇷🇺 Google-перевод'}function update(){fetch('/api/translation-status',{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error();return r.json()}).then(function(data){show(data.mode)}).catch(function(){show('google')})}update();setInterval(update,60000)})();</script>
</body>
</html>`;
}
...
function renderAdminPage({
  ...
  botStatistics = { totals: {}, commands: [], frequency: [], daily: [] },
  statisticsSources = [],
  newsSources = [],
  newsSourceStatus = '',
  duplicateArticles,
  ...
  const breakdownTable = (items, empty) => items?.length
    ? `<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Название</th><th>Статьи</th><th>Доля</th></tr></thead><tbody>${items.map((item) => `<tr><th scope="row">${escapeHtml(item.name)}</th><td>${item.articles}</td><td>${statistics.report.articles ? Math.round(item.articles / statistics.report.articles * 100) : 0}%</td></tr>`).join('')}</tbody></table></div>`
    : `<p class="summary">${escapeHtml(empty)}</p>`;
  const articleBreakdowns = `<div class="admin-grid"><section class="admin-panel"><h2>Статьи по источникам</h2><p class="summary">Распределение публикаций за выбранный период.</p>${breakdownTable(statistics.bySource, 'Нет статей по источникам.')}</section><section class="admin-panel"><h2>Статьи по темам</h2><p class="summary">Какие разделы формируют новостной поток.</p>${breakdownTable(statistics.byCategory, 'Нет статей по темам.')}</section></div>`;
  ...
  const monthlyVisitorMarkup = `<section class="admin-panel admin-panel--wide visitor-overview"><div class="admin-panel-heading"><div><p class="eyebrow">Посещаемость</p><h2>Посетители за месяц</h2></div><dl class="admin-stats admin-stats--monthly"><div class="stat-card"><dt>Уникальных за 30 дней</dt><dd>${statistics.siteVisitorsMonth || 0}</dd></div></dl></div>${visitorChart}</section>${articleBreakdowns}`;
    ...
    <section class="admin-panel telegram-channel-card telegram-channel-settings">
      ...
      <form class="admin-form account-form" action="/admin/telegram-channel/settings" method="post">
        ...
        <div class="account-form-grid">
          ...
          <label class="account-field" for="channel-limit"><span>Страховочный лимит постов в день</span><input id="channel-limit" name="max_posts_per_day" type="number" min="1" max="100" value="${telegramChannelSettings.maxPostsPerDay}" required><small>Это только защита от перегрузки. Основной отбор выполняется по важности новости.</small></label>
          <label class="account-field" for="channel-percent"><span>Доля публикаций от лимита, %</span><input id="channel-percent" name="delivery_percent" type="number" min="1" max="100" value="${telegramChannelSettings.deliveryPercent || 100}" required><small>Например, 30% от лимита 100 = до 30 самых важных новостей в день.</small></label>
        </div>
  ...
  const botTotals = botStatistics.totals || {};
  const botEventLabels = { command_today: '/today — новости сегодня', command_forme: '/forme — для меня', command_hsl: '/hsl — транспорт', command_offers: '/offers — акции', command_saved: '/saved — сохранённые', text_question: 'Вопросы текстом', voice_question: 'Голосовые вопросы', hsl_location: 'Геопозиция HSL', offers_location: 'Геопозиция для акций', button_explain: 'Кнопка «Объяснить»', button_ask: 'Кнопка «Задать вопрос»', button_comment: 'Кнопка «Комментировать»', button_save: 'Кнопка «Сохранить»' };
  const botCommands = botStatistics.commands.length ? `<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Функция</th><th>Использований за 30 дней</th><th>Пользователей</th></tr></thead><tbody>${botStatistics.commands.map((item) => `<tr><td>${escapeHtml(botEventLabels[item.eventType] || item.eventType)}</td><td>${item.uses}</td><td>${item.users}</td></tr>`).join('')}</tbody></table></div>` : '<p class="summary">Использование команд начнёт отображаться после развёртывания этой версии.</p>';
  const botDaily = botStatistics.daily.length ? `<div class="admin-table-scroll"><table class="admin-table"><thead><tr><th>Дата</th><th>Активных пользователей</th><th>Действий</th></tr></thead><tbody>${botStatistics.daily.map((item) => `<tr><td>${escapeHtml(item.day)}</td><td>${item.users}</td><td>${item.actions}</td></tr>`).join('')}</tbody></table></div>` : '<p class="summary">Дневной активности пока нет.</p>';
  const botFrequency = botStatistics.frequency.length ? botStatistics.frequency.map((item) => `<li>${item.frequency === 'instant' ? 'Сразу после публикации' : 'Ежедневная подборка'} <span class="admin-count">${item.users}</span></li>`).join('') : '<li>Активных рассылок пока нет.</li>';
  const botStatisticsMarkup = `<section class="admin-panel admin-panel--wide"><h2>Telegram-бот</h2><p class="summary">Подключения, активность и качество персональной доставки.</p><dl class="admin-stats"><div class="stat-card"><dt>Подключили бота</dt><dd>${botTotals.linkedUsers || 0}</dd></div><div class="stat-card"><dt>Активные подписки</dt><dd>${botTotals.activeSubscriptions || 0}</dd></div><div class="stat-card"><dt>Новые за 7 дней</dt><dd>${botTotals.linked7Days || 0}</dd></div><div class="stat-card"><dt>Активны за 7 дней</dt><dd>${botTotals.activeUsers7Days || 0}</dd></div><div class="stat-card"><dt>Новостей отправлено</dt><dd>${botTotals.newsDelivered || 0}</dd></div><div class="stat-card"><dt>Других рассылок</dt><dd>${botTotals.contentDelivered || 0}</dd></div><div class="stat-card"><dt>Успешно за 7 дней</dt><dd>${botTotals.sent7Days || 0}</dd></div><div class="stat-card"><dt>Ошибок за 7 дней</dt><dd>${botTotals.failed7Days || 0}</dd></div></dl></section><div class="admin-grid"><section class="admin-panel"><h2>Режим рассылки</h2><ol>${botFrequency}</ol></section><section class="admin-panel"><h2>Активность за 14 дней</h2>${botDaily}</section></div><section class="admin-panel admin-panel--wide"><h2>Использование функций</h2>${botCommands}</section>`;
  ...
  const rssRefreshPanel = `<section class="admin-panel admin-panel--wide"><div class="admin-panel-heading"><div><p class="eyebrow">RSS и перевод</p><h2>Получить свежие новости</h2><p class="summary">Будут загружены только новые статьи. Уже сохранённые материалы повторно не переводятся и не расходуют баланс API.</p></div><form action="/admin/rss/refresh" method="post"><button type="submit">↻ Обновить RSS сейчас</button></form></div><form class="admin-form" action="/admin/content-intensity" method="post"><label>Интенсивность новостей, %<input name="percent" type="number" min="1" max="100" value="${Number(contentIntensityPercent) || 100}" required><small class="field-hint">Глобально для сайта: сколько новых материалов переводить и публиковать. При 30% берутся самые свежие и приоритетные.</small></label><button type="submit">Сохранить интенсивность</button></form></section>`;
  ...
  const content = `<div class="admin-wrap">
    ...
    ${tabPanel('bot-stats', 'Статистика бота', botStatisticsMarkup)}
      ...
      <section class="account-card account-card--settings">
        ...
        <form class="account-form" method="post" action="/account/subscription">
          ...
          <div class="account-form-grid">
            ...
            <label class="account-field"><span>Страховочный лимит новостей</span><input name="max_posts_per_day" type="number" min="1" max="100" value="${subscription.maxPostsPerDay}"><small>Это верхняя защита. Бот сначала отбирает только самые важные и интересные новости.</small></label>
            <label class="account-field"><span>Время ежедневной подборки</span><input name="delivery_time" type="time" value="${escapeHtml(subscription.deliveryTimes?.[0] || '19:00')}"><small>Рекомендуем вечерний дайджест в 19:00.</small></label>
          </div>
          <div class="account-callout">
